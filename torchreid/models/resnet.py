...
class ACM(nn.Module):
    def __init__(self, acm_drop_rate=0.75, acm_drop_threshold=0.8):
        super(ACM, self).__init__()
        if not (0 <= acm_drop_rate <= 1):
            raise ValueError("Drop rate must be in range [0, 1].")
        if not (0 <= acm_drop_threshold <= 1):
            raise ValueError("Drop threshold must be in range [0, 1].")
        self.acm_drop_rate = acm_drop_rate
        self.acm_drop_threshold = acm_drop_threshold
        self.attention = None
        self.drop_mask = None
    ...
    def _select_map(self, importance_map, drop_mask):
        random_tensor = torch.rand([], dtype=torch.float32) + self.acm_drop_rate
    ...
    def _drop_mask(self, attention):
        ...
        thr_val = max_val * self.acm_drop_threshold
        thr_val = thr_val.view(b_size, 1, 1, 1)
        return (attention < thr_val).float()

    def extra_repr(self):
        return 'acm_drop_rate={}, acm_drop_threshold={}'.format(
            self.acm_drop_rate, self.acm_drop_threshold)
...
class ResNet(nn.Module):
    ...
    def _construct_fc_layer(self, fc_dims, input_dim, dropout_p=None):
        ...
        for dim in fc_dims:
            ...
            layers.append(ACM(self.acm_drop_rate, self.acm_threshold))              
            input_dim = dim
    ...
    def featuremaps(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
       # x = x.append(ACM(self.acm_drop_rate, self.acm_threshold))
        x = self.layer4(x)
       # x = x.append(ACM(self.acm_drop_rate, self.acm_threshold))
